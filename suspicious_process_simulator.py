...
# 独立したワーカー関数群（multiprocessingで使用）
def fork_worker(depth, worker_id, max_depth, delay):
    """フォーク爆弾のワーカープロセス"""
    if depth >= max_depth:
        return
    ...
    try:
        ...
        if depth < max_depth - 1:
            ...
            for i in range(2):  # 2つの子プロセスを生成
                p = Process(target=fork_worker, args=(depth + 1, i, max_depth, delay))
                p.start()
                processes.append(p)
                time.sleep(delay)
...
def mass_process_worker(worker_id, long_lived_mode=False):
    """大量プロセス生成のワーカープロセス"""
    try:
        # プロセス名を設定
        if hasattr(os, 'prctl'):
...
def name_changer_worker(worker_id):
    """プロセス名変更のワーカープロセス"""
    try:
        suspicious_names = [
...
def memory_worker(worker_id, duration, long_lived_mode=False):
    """メモリ集約的処理のワーカープロセス"""
    try:
        if hasattr(os, 'prctl'):
            import prctl
...
class SuspiciousProcessSimulator:
    ...
    def fork_bomb_controlled(self, max_depth=3, delay=0.5):
        ...
        # 初期プロセス開始
        initial_process = Process(target=fork_worker, args=(0, 0, max_depth, delay))
    ...
    def mass_process_generator(self, count=20, interval=0.2):
        ...
        for i in range(count):
            if not self.running:
                break
            long_lived = hasattr(self, 'long_lived_mode') and self.long_lived_mode
            p = Process(target=mass_process_worker, args=(i, long_lived))
            p.start()
    ...
    def process_name_changer(self, count=5):
        ...
        for i in range(count):
            if not self.running:
                break
            p = Process(target=name_changer_worker, args=(i,))
            p.start()
            self.processes.append(p)
            time.sleep(0.5)
    ...
    def cpu_intensive_workers(self, count=3, duration=10):
        ...
        for i in range(count):
            if not self.running:
                break
            p = Process(target=cpu_worker, args=(i, duration))
            p.start()
            self.processes.append(p)
    ...
    def memory_intensive_workers(self, count=3, duration=30):
        ...
        for i in range(count):
            if not self.running:
                break
            long_lived = hasattr(self, 'long_lived_mode') and self.long_lived_mode
            p = Process(target=memory_worker, args=(i, duration, long_lived))
            p.start()
            self.processes.append(p)
