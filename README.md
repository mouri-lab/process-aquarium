...
- 形状: ブラウザ/開発ツール/システム系など名称パターンで差異（内部規則に基づき決定）
...
- 目的: リストや数値を見なくても「異様に肥大した/影響力の大きい」プロセスを数秒以内に発見できるようにする視覚シグナル

**群れ**
- 目的: 関連するプロセス群を視覚的なまとまりとして把握しやすくするクラスタリング
- 挙動: 距離バランスを保ち整列・凝集する自然な群泳パターン
- 強調 (Q): 群れ外を淡くし本体を際立たせる（分析開始時に有効化推奨）
- 代表的な用途: 大量ワーカー監視 / 短命プロセス生成検知 / 再起動後の再構成観察
- 制限: 親子/通信構造を幾何配置へ直接投影する高度機能は未対応
