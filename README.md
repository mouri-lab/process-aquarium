...
- 左クリック: カーソル下の魚を選択し詳細パネル表示
- 右クリック: その魚を追従対象に設定（再選択で切替）
- マウスホイール: ズーム（最小0.1x〜最大5x）
- 左ドラッグ: 画面パン（追従モード中は無効化される場合あり）
- F / F11: フルスクリーン切替
- Esc: 終了
...
**色 / 形状**
- 初期色: プロセス名のハッシュから決定（CPU使用率で色相は変化しない）
- 形状: ブラウザ/開発ツール/システム系など名称パターンで差異（`fish.py` 内 `_determine_fish_shape`）
...
**スレッド衛星**
- 最大14個の小衛星ドットが周回（スレッド数に応じた配置半径）

**群れ**
- 同名または関係性が検出されたプロセスを `_update_schooling_behavior` でグルーピング
- `highlight_schools` 有効時: 孤立プロセスを半透明化して群れを視認しやすく
- 親子レイアウトの特別な位置補正は現状なし（親PIDは内部保持）

**ライフサイクル**
- 生成直後: スポーンアニメーション（フェード/スケールイン）
- 終了中: フェードアウト（`is_dying`）
- fork/exec: 一時的なグローや色再生成イベント（`set_fork_event`, `set_exec_event`）
